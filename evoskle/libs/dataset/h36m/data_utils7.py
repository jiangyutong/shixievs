...
import libs.model.model2d4  as model
...
def prepare_data_dict(rcams,
    ...
    if opt.train:
        ...
        data_mean_2d_train, data_std_2d_train, dim_to_ignore_2d, dim_to_use_2d = \
        ...
        train_dict_2d_nor = normalize_data(train_dict_2d,
                                                  data_mean_2d_train,
                                                  data_std_2d_train,
                                                  dim_to_use_2d,
    ...
    complete_train_2d = copy.deepcopy(np.vstack(list(test_dict_2d.values())))
    ...
    test_dict_2d_nor = normalize_data(test_dict_2d,
                                       data_mean_2d,
                                       data_std_2d,
                                       dim_to_use_2d,
                                       norm_single=opt.norm_single
    ...
    train_dict_2d_targ = project_to_cameras2(train_dict_3d, rcams, ncams=ncams)
    complete_train_2d = copy.deepcopy(np.vstack(list(train_dict_2d_targ.values())))
    data_mean_2d, data_std_2d, dim_to_ignore_2d, dim_to_use_2d = \
        normalization_stats(complete_train_2d,
                            dim=2,
                            norm_twoD=opt.norm_twoD,
    ...
    train_dict_2d_targ_nor = normalize_data(train_dict_2d_targ,
                                      data_mean_2d,
                                      data_std_2d,
                                      dim_to_use_2d,
                                      norm_single=opt.norm_single
    ...
    test_dict_2d_targ = project_to_cameras2(test_dict_3d, rcams, ncams=ncams)
    complete_train_2d = copy.deepcopy(np.vstack(list(test_dict_2d_targ.values())))
    data_mean_2d, data_std_2d, dim_to_ignore_2d, dim_to_use_2d = \
        normalization_stats(complete_train_2d,
                            dim=2,
                            norm_twoD=opt.norm_twoD,
    ...
    test_dict_2d_targ_nor = normalize_data(test_dict_2d_targ,
                                            data_mean_2d,
                                            data_std_2d,
                                            dim_to_use_2d,
                                            norm_single=opt.norm_single
    ...
    train_dict_3d_convert = transform_world_to_camera(train_dict_3d, rcams, ncams=ncams)
    # apply 3d post-processing (centering around root)
    train_dict_3d_convert, train_root_positions = postprocess_3d(train_dict_3d_convert)
    complete_train_2d = copy.deepcopy(np.vstack(list(train_dict_3d_convert.values())))
    data_mean_2d, data_std_2d, dim_to_ignore_2d, dim_to_use_2d = \
        normalization_stats(complete_train_2d,
                            dim=2,
                            norm_twoD=opt.norm_twoD,
    ...
    train_dict_3d_convert_nor = normalize_data(train_dict_3d_convert,
                                            data_mean_2d,
                                            data_std_2d,
                                            dim_to_use_2d,
                                            norm_single=opt.norm_single
    ...
    # test_dict_3d_convert = transform_world_to_camera(test_dict_3d, rcams, ncams=ncams)
    # complete_train_2d = copy.deepcopy(np.vstack(list(test_dict_3d_convert.values())))
    # data_mean_2d, data_std_2d, dim_to_ignore_2d, dim_to_use_2d = \
    #     normalization_stats(complete_train_2d,
    #                         dim=2,
    #                         norm_twoD=opt.norm_twoD,
    ...
    #                         )
    #
    # test_dict_3d_convert_nor = normalize_data(test_dict_3d_convert,
    #                                            data_mean_2d,
    #                                            data_std_2d,
    #                                            dim_to_use_2d,
    #                                            norm_single=opt.norm_single
    ...
    #
    # j = 0
    # for key1, key2 in zip(sorted(train_dict_2d_nor.keys()), sorted(train_dict_2d_targ_nor.keys())):
    #     tran_all.append(train_dict_2d_nor[key1])
    #     target_all.append(train_dict_2d_targ_nor[key2])
    #     threeDcameras_all.append(train_dict_3d_convert_nor[key2])
    #     R, T, f, c, k, p, name = rcams[(key2[0], j % 4 + 1)]
    #     j = j + 1
    #     for i in range(train_dict_2d_targ[key2].shape[0]):
    #         f_all.append(f)
    ...
    # dataloader2 = DataLoader(dataset=deal_dataset2, batch_size=1024, shuffle=True)
    # deal_dataset = trainset(target=target_all, data=tran_all)
    # dataloader = DataLoader(dataset=deal_dataset, batch_size=8192, shuffle=True)
    # opt = parse.parse_arg()
    # opt.cuda = opt.gpuid >= 0
    # mymodel = model.get_model(
    #                           refine_3d=opt.refine_3d,
    ...
    #         # forward pass to get prediction
    #         prediction = mymodel(data_train)
    #         threeDcameras_cum = threeDcameras.reshape(-1, 17, 3)
    #         prediction_cum = prediction.reshape(-1, 17, 2)
    #         target_cum = data_targ.reshape(-1, 17, 2)
    #         XdivZ = threeDcameras_cum[:, :, 0] / threeDcameras_cum[:, :, 2]
    #         YdivZ = threeDcameras_cum[:, :, 1] / threeDcameras_cum[:, :, 2]
    #         XX = torch.stack((XdivZ, YdivZ), 2)
    #         XX = XX.cuda()
    ...
    #         loss.backward()
    #         optim.step()
    #     print("Epoch:{} Loss：{} loss3:{} ".format(batch_idx, loss, loss4))
    #
    #         # forward pass to get predictio
    # test_dict_2d_pred={}
    # train_dict_2d_pred = {}
    # for key in train_dict_2d_nor.keys():
    #     #         # for i in range(50):
    #     data_train = train_dict_2d_nor[key]
    #     data_train = torch.Tensor(data_train)
    #     data_train = data_train.cuda()
    #     # mymodel = mymodel.cpu()
    #     prediction = mymodel(data_train)
    ...
    #     keynew.append(key2)
    #     keynew = tuple(keynew)
    #     data_target = train_dict_2d_targ_nor[keynew]
    #     # print("Key: {} datshape:{} targtshape:{} ".format(key, data_train.shape, data_target.shape))
    #     data_target = torch.Tensor(data_target)
    #     data_target = data_target.cuda()
    #     loss = F.mse_loss(prediction, data_target)
    ...
    #     train_dict_2d_pred[key] = prediction
    #     # mymodel.eval()
    # for key in test_dict_2d_nor.keys():
    #     data_train = test_dict_2d_nor[key]
    #     data_train = torch.Tensor(data_train)
    #     data_train = data_train.cuda()
    #     # mymodel = mymodel.cpu()
    #     prediction = mymodel(data_train)
    ...
    #     keynew.append(key2)
    #     keynew = tuple(keynew)
    #     data_target = test_dict_2d_targ_nor[keynew]
    #     # print("Key: {} predictionshape:{} targtshape:{} ".format(key, prediction.shape, data_target.shape))
    #     data_target = torch.Tensor(data_target)
    #     data_target = data_target.cuda()
    #     loss = F.mse_loss(prediction, data_target)
    ...
    # #     train_dict_2d_pred[key]=train_dict_2d_pred[key][:,dim_to_use_2d]
    # # for key in test_dict_2d_pred.keys():
    # #     test_dict_2d_pred[key] = test_dict_2d_pred[key][:,dim_to_use_2d]
    data_dic['train_set_2d']=train_dict_2d
    data_dic['test_set_2d'] = test_dict_2d
    ...
    if opt.train:
        ...
        dim_use_2d = list_remove([i for i in range(len(data_mean_2d_train))],
        ...
        data_stats = {'mean_2d':data_mean_2d_train,
                      'std_2d':data_std_2d_train,
