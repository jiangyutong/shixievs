...
import libs.model.model2d2  as model
...
def get_dict_2d(train_dict_3d, test_dict_3d, rcams, ncams, opt):
    ...
    dataloader2 = DataLoader(dataset=deal_dataset2, batch_size=8192, shuffle=True)
    ...
    train_dict_2d_pred = {}
    test_dict_2d_pred = {}
    mymodel = torch.load('/media/zlz422/82BEDFE2BEDFCD33/jyt/project/EvSkeleton/data/human3.6M/2021-10-31 21:47:21.026788_jyt_fine_2d_zor/model')
    now = datetime.datetime.now()
    print("******************************now:{}***************************".format(now))
    torch.cuda.set_device(0)
    mymodel=mymodel.cuda()
    # mymodel = torch.load(
    #     '/media/zlz422/82BEDFE2BEDFCD33/jyt/project/EvSkeleton/data/human3.6M/2021-10-31 22:47:00.333293_jyt_fine_2d_zor/model')
    # mymodel.train()
    ...
    #
    #         loss = 0.01*F.smooth_l1_loss(f,fpred)+F.smooth_l1_loss(prediction,data_targ)
    #
    #         loss.backward()
    #         optim.step()
    #         lossall = lossall + loss
    ...
    # now = datetime.datetime.now()
    # stpath = str(now) + "_" + "jyt_fine_2d_2Dto3D2"
    # save2Dpath = os.path.join(opt.data_dir, stpath)
    # if not os.path.exists(save2Dpath):
    #     os.mkdir(save2Dpath)
    # save2Dpath = save2Dpath + "/" + "model"
    ...
    for key in train_dict_2d.keys():
        ...
        print("key:{} TrainLoss: {}".format(key, loss))
    ...
    for key in test_dict_2d.keys():
        ...
        data_target = data_target.cuda()
        loss = F.mse_loss(prediction, data_target)
        print("key:{} TestLoss: {}".format(key, loss))
        prediction = prediction.detach().cpu().numpy()

        # prediction = [ predictionitem.detach().numpy() for predictionitem in prediction]
        test_dict_2d_pred[key] = prediction

    save_path = opt.data_dir+str(now)+"2drefinetrainlast"
    np.save(save_path,train_dict_2d)
    save_path = opt.data_dir+str(now) + "2drefinetestlast"
