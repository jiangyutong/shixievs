...
import libs.model.mode2d6  as model
...
def get_dict_2d(train_dict_3d, test_dict_3d, rcams, ncams, opt):
    ...
    # # init_distributed_mode(args=myopt)
    # rank = myopt.rank
    # device = torch.device(myopt.device)
    # batch_size = myopt.batch_size
    # weights_path = myopt.weights
    # myopt.lr *= myopt.world_size  # 学习率要根据并行GPU的数量进行倍增
    ...
    #
    train_dict_2d_targ = project_to_cameras(train_dict_3d, rcams, ncams=ncams)
    test_dict_2d_train = {}
    test_dict_2d_targ = project_to_cameras(test_dict_3d, rcams, ncams=ncams)
    train_dict_2d_train = {}
    tran_all = []
    target_all = []
    threeDcameras_all=[]
    ...
    j = 0
    dim_twouse=dimUse(dim=2)
    dim_threeuse = dimUse(dim=3)
    for key1, key2 in zip(sorted(train_dict_2d.keys()), sorted(train_dict_2d_targ.keys())):
        tran_all.append(train_dict_2d[key1][:,dim_twouse])
        target_all.append(train_dict_2d_targ[key2][:,dim_twouse])
        threeDcameras_all.append(train_dict_3d_convert[key2][:,dim_threeuse])
    ...
    dataloader2 = DataLoader(dataset=deal_dataset2, batch_size=8192, shuffle=True,pin_memory=True,num_workers=32)
    ...
    # device_ids = [0,1]
    # mymodel = torch.nn.DataParallel(mymodel, device_ids=device_ids)  # 声明所有可用设备
    # mymodel = mymodel.cuda(device=device_ids[0])
    mymodel.cuda()
    # mymodel = nn.DataParallel(mymodel)
    # optim = torch.optim.SGD(mymodel.parameters(), lr=myopt.lr, momentum=0.9, weight_decay=0.005)
    # Scheduler https://arxiv.org/pdf/1812.01187.pdf
    optim = torch.optim.Adam(mymodel.parameters(), lr=0.002, weight_decay=0.0)
    # lf = lambda x: ((1 + math.cos(x * math.pi / myopt.epochs)) / 2) * (1 - myopt.lrf) + myopt.lrf  # cosine
    ...
    for batch_idx in range(1, epochs + 1):
        ...
        lossall=0
        # scheduler.step()
        # if batch_idx<20 or batch_idx>myopt.epochs-20:
        ...
        # for i, (data_targ, data_train,threeDcameras,f,c) in enumerate(dataloader2):
            ...
            target_cum = data_targ.reshape(-1, 16, 2)
            ...
            data_targ, data_train, threeDcameras, f, c = prefetcher.next()
            # scheduler.step()
        print("Epoch:{} LossAll：{} Loss：{} loss3:{} ".format(batch_idx,lossall/iteration,loss,loss4))

    now = datetime.datetime.now()
    stpath = str(now) + "_" + "jyt_fine_2d_del"
    ...
    for key in test_dict_2d.keys():
        ...
        data_train = test_dict_2d[key][:,dim_twouse]
        keynew = []
        keynew.append(key[0])
        keynew.append(key[1])
        ...
        keynew.append(key2)
        keynew = tuple(keynew)
        data_target = test_dict_2d_targ[keynew][:, dim_twouse]
        data_train = torch.Tensor(data_train)
    ...
    for key in train_dict_2d.keys():
        ...
        data_train = train_dict_2d[key][:,dim_twouse]
        keynew = []
        keynew.append(key[0])
        keynew.append(key[1])
        key2 = key[2].replace("-sh", "")
        keynew.append(key2)
        keynew = tuple(keynew)
        data_target = train_dict_2d_targ[keynew][:, dim_twouse]
        data_train = torch.Tensor(data_train)
        data_target = torch.Tensor(data_target)
        if opt.cuda:
            with torch.no_grad():
...
def prepare_data_dict(rcams,
    ...
    if opt.train:
        ...
        data_dic['train_set_2d'] = twonormalize(train_dict_2d)
    else:
    ...
    data_dic['test_set_2d']  = twonormalize(test_dict_2d)
...
def transform_world_to_camera(poses_set, cams, ncams=4):
    ...
    return t3d_camera
def twonormalize(data):
    complete_data = copy.deepcopy(np.vstack(list(data.values())))
    data_mean = np.mean(complete_data, axis=0)
    data_std = np.std(complete_data, axis=0)
    data_out = {}
    for key in data.keys():
