...
import libs.model.model12  as model
...
class DataPrefetcher2():
    ...
    def preload(self):
        try:
            self.data_targ, self.data_train= next(self.loader)
        except StopIteration:
            self.data_targ = None
            self.data_train = None
...
def get_dict_2d(train_dict_3d, test_dict_3d, rcams, ncams, opt):
    ...
    dataloader = DataLoader(dataset=deal_dataset, batch_size=1024, shuffle=True)
    ...
    mymodel = torch.load('/media/zlz422/82BEDFE2BEDFCD33/jyt/project/EvSkeleton/data/human3.6M/2021-11-25 02:47:20.079523_jyt_fine_2d_fandcandtarg/model')
    # mymodel = torch.load(
    #     '/media/zlz422/82BEDFE2BEDFCD33/jyt/project/EvSkeleton/data/human3.6M/2021-11-01 01:46:33.880597_jyt_fine_2d_zor/model')
    #
    epoch = 30
    # device_ids = [1]  # id为0和1的两块显卡
    # # # model = torch.nn.DataParallel(model, device_ids=device_ids)
    # mymodel = torch.nn.DataParallel(mymodel, device_ids=device_ids).cuda()
    mymodel = mymodel.cuda()
    # optim = torch.optim.Adamax(mymodel.parameters(), lr=0.001, weight_decay=0.0002)
    optim = torch.optim.Adamax(mymodel.parameters(), lr=0.004, weight_decay=0.0002)

    for batch_idx in range(1, epoch + 1):
        if batch_idx == 6:
            adjust_lr(optim, 0.003)
        if batch_idx == 9:
            adjust_lr(optim, 0.002)
        if batch_idx == 15:
            adjust_lr(optim, 0.001)
        if batch_idx == 20:
            adjust_lr(optim, 0.0005)
        if batch_idx == 9:
            adjust_lr(optim, 0.01)
        ...
        while data_targ is not None:
            ...
            fm = (torch.sum(prediction_cum * XX, axis=1) - 32 * (
                    torch.mean(prediction_cum, axis=1) * torch.mean(XX, axis=1)))
            fpred = fm / fz
            ...
            f = f.squeeze()
            c = c.squeeze()
            loss11 = F.mse_loss(f, fpred)
            loss12 = F.mse_loss(c, cpred)
            loss4 = F.mse_loss(prediction, data_targ)
            # loss1 = F.smooth_l1_loss(f, fpred)
            # loss2 = F.smooth_l1_loss(c, cpred)
            # loss3 = F.smooth_l1_loss(prediction, data_targ)
            # if batch_idx>50:
            #     loss = loss11
            # else:
            loss= 0.1*F.smooth_l1_loss(f, fpred)+F.smooth_l1_loss(prediction, data_targ)

            loss.backward()
            optim.step()
            lossall = lossall + loss
            loss4all = loss4all + loss4
            # lossfall += loss11 + lossfall
            losscall += loss12
            data_targ, data_train, threeDcameras, f, c = prefetcher.next()
        print("Epoch:{} Lossall:{} Loss：{} lossf:{} lossc:{} loss3:{} ".format(batch_idx, lossall / iteration, loss,
                                                                               loss11, losscall / iteration,
                                                                               loss4all / iteration))
    # # # mymodel = torch.load(
    #     # '/media/zlz422/82BEDFE2BEDFCD33/jyt/project/EvSkeleton/data/human3.6M/2021-11-18 12:15:23.641160_jyt_fine_2d_every/model')
    # mymodel.train()
    ...
    #     #     adjust_lr(optim, 0.001)
    #     # if batch_idx == 30:
    #     #     adjust_lr(optim, 0.0008)
    #     # if batch_idx == 40:
    #     #     adjust_lr(optim, 0.0006)
    #     if batch_idx == 20:
    #         adjust_lr(optim, 0.008)
    #     if batch_idx == 40:
    ...
    #             with torch.no_grad():
    #                 # move to GPU
    #                 data_train = data_train.cuda(non_blocking=True)
    #                 data_targ = data_targ.cuda(non_blocking=True)
    #                 threeDcameras = threeDcameras.cuda(non_blocking=True)
    ...
    #         loss12 = F.mse_loss(c, cpred)
    #         loss4 = F.mse_loss(prediction, data_targ)
    #         lossf = F.smooth_l1_loss(f, fpred)
    #         lossv = F.smooth_l1_loss(prediction, data_targ)
    #         # loss1 = F.smooth_l1_loss(f, fpred)
    #         # loss2 = F.smooth_l1_loss(c, cpred)
    #         # loss3 = F.smooth_l1_loss(prediction, data_targ)
    #         # loss = xshu * loss11 + xshutwo*loss4
    #         loss = 2*lossf+lossv
    #         loss.backward()
    #         optim.step()
    #         lossall = lossall + loss
    ...
    # epoch = 10
    # mymodel = torch.load(
    #     '/media/zlz422/82BEDFE2BEDFCD33/jyt/project/EvSkeleton/data/human3.6M/2021-11-21 21:02:44.268716_jyt_fine_2d_fandcandtarg/model')
    ...
    # mymodel = mymodel.cuda()
    # epoch =10
    # optim = torch.optim.Adamax(mymodel.parameters(), lr=0.0002, weight_decay=0.0002)
    # for batch_idx in range(1, epoch + 1):
    #     print("lr:{}".format(optim.state_dict()['param_groups'][0]['lr']))
    #     lossall = 0
    #     if batch_idx == 4:
    ...
    #     if batch_idx == 40:
    #         adjust_lr(optim,0.0005)
    #     prefetcher = DataPrefetcher2(dataloader)
    #     data_targ, data_train = prefetcher.next()
    #     iteration = 0
    #     while data_targ is not None:
    #         iteration += 1
    #         # 训练代码
    ...
    #         optim.step()
    #         data_targ, data_train = prefetcher.next()
    #     print("Epoch:{} Lossall:{}  Loss：{}".format(batch_idx, lossall/iteration,loss))
    # epoch = 50
    # mymodel = torch.load(
    #     '/media/zlz422/82BEDFE2BEDFCD33/jyt/project/EvSkeleton/data/human3.6M/2021-11-21 17:45:25.299557_jyt_fine_2d_fandcandtarg/model')
    #
    # # mymodel = torch.load(
    # # '/media/zlz422/82BEDFE2BEDFCD33/jyt/project/EvSkeleton/data/human3.6M/2021-11-18 12:15:23.641160_jyt_fine_2d_every/model')
    # device_ids = [0, 1]  # id为0和1的两块显卡
    ...
    # mymodel.train()
    # # mymodel = mymodel.cuda()
    # optim = torch.optim.Adamax(mymodel.parameters(), lr=0.0004)
    # j = 0
    # # for m in range(3):
    # #     if m==1:
    # #         print("m:::::::::::::::{}".format(m))
    ...
    #     print("*********************************i:{}***********************************".format(i))
    #     if i==3:
    #         adjust_lr(optim, 0.0003)
    #     if i == 5:
    #         adjust_lr(optim, 0.0003)
    #     if i == 10:
    #         adjust_lr(optim, 0.0001)
    #     if i == 20:
    #         adjust_lr(optim, 0.00002)
    #         print("lr:{}".format(optim.state_dict()['param_groups'][0]['lr']))
    #     if i==35:
    #         adjust_lr(optim, 0.00001)
    #         print("lr:{}".format(optim.state_dict()['param_groups'][0]['lr']))
    #     for key in random_dic(train_dict_2d_targ):
    #         if key[1] == "SittingDown" or key[1] == "Sitting":
    #             epoch = 80
    #         else:
    #             epoch = 40
    #         lossall = 0
    ...
    #             #         print("lr:{}".format(optim.state_dict()['param_groups'][0]['lr']))
    #             #     if batch_idx == 20:
    #             #         adjust_lr(optim, 0.0004)
    #             #         print("lr:{}".format(optim.state_dict()['param_groups'][0]['lr']))
    #             #     if batch_idx == 30:
    #             #         adjust_lr(optim, 0.0002)
    #             #         print("lr:{}".format(optim.state_dict()['param_groups'][0]['lr']))
    #             #     # if batch_idx == 9000:
    #             #     #     adjust_lr(optim, 0.01)
    #             #     #     print("lr:{}".format(optim.state_dict()['param_groups'][0]['lr']))
    ...
    #                     data_targ = torch.Tensor(data_targ)
    #                     threeDcameras = torch.Tensor(threeDcameras)
    #                     data_train, data_targ, threeDcameras = data_train.cuda(non_blocking=True), data_targ.cuda(non_blocking=True), threeDcameras.cuda(non_blocking=True)
    #             optim.zero_grad()
    #             # forward pass to get prediction
    #             prediction = mymodel(data_train)
    #             # compute loss
    ...
    #             # cpred_target = torch.mean(cpred_target,dim=0)
    #             fz = (torch.sum(XX * XX, axis=0) - n_smaples * (torch.mean(XX, axis=0) * torch.mean(XX, axis=0)))
    #             fm = (torch.sum(prediction_cum * XX, axis=0) - n_smaples * (
    #                         torch.mean(prediction_cum, axis=0) * torch.mean(XX, axis=0)))
    #             fpred = fm / fz
    ...
    #             # loss = 0.2 * loss1 + 0.1 * loss2+0.05*loss3
    #             lossall = lossall + loss
    #             loss.backward()
    #             optim.step()
    #             # logging
    ...
    #                 print("[{}] key:{} f: {} c {}  prdef :{} predc :{} Train Epoch: {} Loss:{:.3f}".format(now, key,
    #                                                                                                        f.detach().cpu().numpy(),
    #                                                                                                        c.detach().cpu().numpy(),
    #                                                                                                        fpred.detach().cpu().numpy(),
    #                                                                                                        cpred.detach().cpu().numpy(),
    #                                                                                                        batch_idx,
    #                                                                                                        loss))
        # epoch = 500
    # scheduler = torch.optim.lr_scheduler.StepLR(optim, step_size=10, gamma=0.9)
    # optim = torch.optim.Adamax(mymodel.parameters(), lr=0.01, weight_decay=0.0002)
    # j = 0
    # for key in train_dict_2d_targ.keys():
    #     lossall = 0
    #     # scheduler.step()
    ...
    #         # loss3 = F.mse_loss(prediction, data_targ)
    #         # loss=0.1*loss2+0.1*loss1+0.9*loss3
    #         loss = 0.01 * loss1 + 0.01 * loss2+0.2*loss3
    #         lossall = lossall + loss
    #         loss.backward()
    #         optim.step()
    #         # logging
    ...
    #             optim.step()
    #             if batch_idx%20==0:
    #                 print("key[{}] Epoch:{} Loss：{}".format(key,batch_idx, loss))
    #             j=j+1

            # prediction = [ predictionitem.detach().numpy() for predictionitem in prediction]
    # epoch =100
    # optim = torch.optim.Adamax(mymodel.parameters(), lr=0.002, weight_decay=0.0002)
    # for batch_idx in range(1, epoch + 1):
    #     print("lr:{}".format(optim.state_dict()['param_groups'][0]['lr']))
    #     lossall = 0
    #     if batch_idx == 30:
    ...
    #     if batch_idx % 100 == 0:
    #         now = datetime.datetime.now()
    #         stpath = str(now) + "_" + "jyt_fine_2d_evem" + str(epoch)
    ...
    #         print("保存啦啦啦啦啦啦啦啦啦啦啦啦:{}".format(epoch))
    #     if batch_idx == 10:
    #         adjust_lr(optim, 0.0008)
    #     if batch_idx == 20:
    #         adjust_lr(optim, 0.0007)
    #     if batch_idx == 25:
    #         adjust_lr(optim, 0.0006)
    #     if batch_idx == 30:
    #         adjust_lr(optim, 0.0005)
    ...
    #         optim.step()
    #         data_targ, data_train, threeDcameras, f, c = prefetcher.next()
    #     print("Epoch:{} LossAll：{} loss3:{} ".format(batch_idx,lossall/iteration,loss3all/iteration))
    #
    #
    # optim = torch.optim.Adamax(mymodel.parameters(), lr=0.0005, weight_decay=0.0002)
    # epoch = 2
    # epoch2 = 50
    # epoch3 = 50
    #
    ...
    #             f = f.cuda()
    #             c = c.cuda()
    #             loss2=F.mse_loss(f, fpred)
    #             loss21 = F.mse_loss(c, cpred)
    #             loss1 = F.mse_loss(prediction, data_targ)
    #             # loss1 = F.smooth_l1_loss(f, fpred)
    #             # loss2 = F.smooth_l1_loss(c, cpred)
    #
    #             # loss = F.mse_loss(prediction, data_targ)
    ...
    #             # logging
    #             # if batch_idx % 30 == 0:
    #             if m == 0 or batch_idx2==epoch2-1:
    #                 now = datetime.datetime.now()
    #                 print(
    #                     "[{}] key:{} Train Epoch: {}  train2:{} Loss: {} Lossf:{} Lossc:{}".format(now, key,batch_idx,batch_idx2,loss,loss1,loss2))
    #                 # print("Train target: f:{} c:{}   real target: f:{} c:{}".format(fpred_target, cpred_target, f, c))
    #                 # print("[{}] key:{} Train Epoch: {}  train2:{} Loss:  {} Lossf:{} Lossc:{} Loss3: {}".format(now, key,batch_idx, batch_idx2,loss,loss11,loss21,loss31))
    #                 m = m+1
    #
    ...
    #         m = 0  # print("[{}] key:{} f: {} c {}  prdef :{} predc :{} Train Epoch: {} Loss:{:.3f}".format(now,key,f.detach().cpu().numpy(),c.detach().cpu().numpy(),fpred.detach().cpu().numpy(),cpred.detach().cpu().numpy(),batch_idx,loss))
    #         j = j + 1
    #     if j%200==0:
    # #         now = datetime.datetime.now()
    # #         stpath = str(n(prediction, data_targ)
    #     #         loss.backward()
    #     #         lossall += loss
    #     #         optim.step()ow) + "_" + "jyt_fine_2d_mid"
    # #         save2Dpath = os.path.join(opt.data_dir, stpath)
    # #         if not os.path.exists(save2Dpath):
    # #             os.mkdir(save2Dpath)
    # #         save2Dpath = save2Dpath + "/" + "model"
    ...
    #         loss = F.smooth_l1_loss(prediction, data_target)
    #         loss.backward()
    #         optim.step()
    #         # if i%5==0:
    ...
    #         # if i == 49:
    #         #     loss2 = F.mse_loss(prediction, data_target)
    #         print("key:{}   FTrainLoss: {}  loss2:{}".format(key, loss,loss2))


    now = datetime.datetime.now()
    stpath = str(now) + "_" + "jyt_fine_2d_fandcandtarg"
    save2Dpath = os.path.join(opt.data_dir, stpath)
    if not os.path.exists(save2Dpath):
        os.mkdir(save2Dpath)
    save2Dpath = save2Dpath + "/" + "model"
    ...
    for key in test_dict_2d.keys():
        ...
        # mymodel=mymodel.cuda()
        mylist = []
        numbeone = int(test_dict_2d[key].shape[0] / 3)
        numbetwo = int(test_dict_2d[key].shape[0] / 3 * 2)
        for i in range(3):
            if i == 0:
                data_train = test_dict_2d[key][0:numbeone]
                data_target = test_dict_2d_targ[keynew][0:numbeone]
            if i == 1:
                data_train = test_dict_2d[key][numbeone:numbetwo]
                data_target = test_dict_2d_targ[keynew][numbeone:numbetwo]
            if i == 2:
                data_train = test_dict_2d[key][numbetwo:test_dict_2d[key].shape[0]]
                data_target = test_dict_2d_targ[keynew][numbetwo:test_dict_2d[key].shape[0]]
            data_train = torch.Tensor(data_train)
            data_train = data_train.cuda()
            # mymodel = mymodel.cpu()
            prediction = mymodel(data_train)
            ...
            # prediction = [ predictionitem.detach().numpy() for predictionitem in prediction]
        mylist = np.concatenate(mylist, axis=0)
        test_dict_2d_pred[key] = mylist
    for key in test_dict_2d.keys():
        keynew = []
        keynew.append(key[0])
        keynew.append(key[1])
    ...
    # save_path = opt.data_dir + str(now) + "2drefinetrainnet"
    # np.save(save_path, train_dict_2d_pred)
    # save_path = opt.data_dir + str(now) + "2drefinetestnet"
    ...
    return train_dict_2d, test_dict_2d_pred, train_dict_3d
