...
import libs.model.model2d  as model
...
def get_dict_2d(train_dict_3d, test_dict_3d, rcams, ncams, opt):
    ...
    dataloader2 = DataLoader(dataset=deal_dataset2, batch_size=4096, shuffle=True)
    ...
    train_dict_2d_pred = {}
    test_dict_2d_pred = {}
    mymodel = torch.load('/media/zlz422/82BEDFE2BEDFCD33/jyt/project/EvSkeleton/data/human3.6M/2021-11-11 11:19:59.614266_jyt_fine_2d_del/model')
    ...
    #                 #     print("lr:{}".format(optim.state_dict()['param_groups'][0]['lr']))
    #             # if j==30:
    #             #         adjust_lr(optim, 0.009)
    #             # else:
    #             #     adjust_lr(optim, 0.08)
    #             #     print("lr:{}".format(optim.state_dict()['param_groups'][0]['lr']))
    #             # if batch_idx == 6000:
    #             #     adjust_lr(optim, 0.06)
    #             #     print("lr:{}".format(optim.state_dict()['param_groups'][0]['lr']))
    #
    #
    #             #     print("4000lr:{}".format(optim.state_dict()['param_groups'][0]['lr']))
    #             # if batch_idx == 5000:
    #             #     adjust_lr(optim, 0.0000001)
    #             #     print("10000lr:{}".format(optim.state_dict()['param_groups'][0]['lr']))
    #             # if batch_idx == 8000:
    #             #     adjust_lr(optim, 0.00000001)
    #             #     print("10000lr:{}".format(optim.state_dict()['param_groups'][0]['lr']))
    ...
    #         loss = 2*F.smooth_l1_loss(f, fpred)+F.smooth_l1_loss(prediction, data_targ)
    #
    #         loss.backward()
    #         optim.step()
    #         lossall = lossall + loss
    ...
    #
    #
    # optim = torch.optim.Adamax(mymodel.parameters(), lr=0.0001, weight_decay=0.0002)
    # epoch = 10
    # epoch2 = 50
    # epoch3 = 50
    # mylr = 0.0001
    ...
    #         if loss2 > 200:
    #             for batch_idx2 in range(0, epoch3):
    #                 data_train = train_dict_2d_train[key]
    #                 data_targ = train_dict_2d_targ[key]
    #                 threeDcameras = train_dict_3d_convert[key]
    #                 # get2ddata(mymodel, optim,data_train,data_targ,threeDcameras,opt,j,epoch,key)
    ...
    #                 XX = XX.cuda()
    #                 fz = (torch.sum(XX * XX, axis=0) - n_smaples * (torch.mean(XX, axis=0) * torch.mean(XX, axis=0)))
    #                 fm = (torch.sum(target_cum * XX, axis=0) - n_smaples * (
    #                         torch.mean(target_cum, axis=0) * torch.mean(XX, axis=0)))
    #                 fpred_target = fm / fz
    #                 cpred_target = torch.mean(target_cum, dim=0) - torch.mean(XX, dim=0) * fpred_target
    #                 fpred_target = torch.mean(fpred_target, dim=0)
    #                 cpred_target = torch.mean(cpred_target, dim=0)
    ...
    #                     now = datetime.datetime.now()
    #                     print(
    #                         "[{}] 800 ::::::key:{} Train Epoch: {}  train2:{} Loss: {} Lossf:{} Lossc:{}".format(now,
    #                                                                                                              key,
    #                                                                                                              batch_idx,
    #                                                                                                              batch_idx2,
    #                                                                                                              loss,
    ...
    # now = datetime.datetime.now()
    # stpath = str(now) + "_" + "jyt_fine_2d_2Dto3D3"
    # save2Dpath = os.path.join(opt.data_dir, stpath)
    # if not os.path.exists(save2Dpath):
    #     os.mkdir(save2Dpath)
    # save2Dpath = save2Dpath + "/" + "model"
    ...
    for key in train_dict_2d.keys():
        ...
        keynew = tuple(keynew)
        mylist=[]
        numbeone=int(train_dict_2d[key].shape[0]/3)
        numbetwo = int(train_dict_2d[key].shape[0] / 3 *2)
        for i in range(3):
            if i==0:
                data_train=train_dict_2d[key][0:numbeone]
                data_target = train_dict_2d_targ[keynew][0:numbeone]
            if i==1:
                data_train=train_dict_2d[key][numbeone:numbetwo]
                data_target = train_dict_2d_targ[keynew][numbeone:numbetwo]
            if i==2:
                data_train=train_dict_2d[key][numbetwo:train_dict_2d[key].shape[0]]
                data_target = train_dict_2d_targ[keynew][numbetwo:train_dict_2d[key].shape[0]]
            data_train = torch.Tensor(data_train)
            data_train = data_train.cuda()
            # mymodel = mymodel.cpu()
            prediction = mymodel(data_train)
    ...
    return train_dict_2d_pred, test_dict_2d_pred, train_dict_3d
