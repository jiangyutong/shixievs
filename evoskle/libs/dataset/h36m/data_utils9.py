...
class DataPrefetcher2():
    ...
    def next(self):
        ...
        self.preload()
        return  data_targ,data_train,threeDcameras
def get_dict_2d(train_dict_3d, test_dict_3d, rcams, ncams, opt):
    ...
    for batch_idx in range(1, epoch + 1):
        ...
        data_targ, data_train, threeDcameras = prefetcher.next()
        iteration = 0
        ...
        for key in train_dict_2d_train.keys():
            ...
            # print("lr:{}".format(optim.state_dict()['param_groups'][0]['lr']))
                ...
                if m == 0 or batch_idx2==epoch2-1:
                    ...
                    print("[{}] key:{} Train Epoch: {}  train2:{} Loss:  {} Loss3: {}".format(now, key,batch_idx, batch_idx2,loss,loss3))
                    m = m+1
    ...
    #     print("Epoch:{} Loss：{}".format(batch_idx, loss))
    now = datetime.datetime.now()
    stpath = str(now) + "_" + "jyt_fine_2d_every"
    save2Dpath = os.path.join(opt.data_dir, stpath)
    if not os.path.exists(save2Dpath):
        os.mkdir(save2Dpath)
    save2Dpath = save2Dpath + "/" + "model"
