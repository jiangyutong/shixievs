...
class FCModel(nn.Module):
    def __init__(self,
        ...
        self.wlstm = nn.Linear(self.input_size*2, self.input_size)
        ...
        self.lstm1 = nn.LSTM(32, 16,batch_first=True,bidirectional=True,num_layers=2)
    ...
    def forward(self, x):
        ...
        h0 = torch.randn(4, y2.shape[0], 16).cuda()
        c0 = torch.randn(4, y2.shape[0], 16).cuda()
        # output(seq_len, batch, hidden_size * num_directions)
        y2 = self.lstm1(y2,(h0,c0))
        # y2 = y2[0].transpose(1, 2)
        y2 = y2[0].reshape(-1,64)
        # y2=self.wlstm(y2)
        y = x + y2
        y1 = y.reshape(-1, 32, 2)
        y1 = y1.transpose(1, 2)
