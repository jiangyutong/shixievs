...
class SpatialAttention(nn.Module):
    def __init__(self, kernel_size=7):
        ...
        self.conv1 = nn.Conv2d(64, 64, kernel_size, padding=padding, bias=False)
        self.sigmoid = nn.Sigmoid()
...
class FCModel(nn.Module):
    def __init__(self,
        ...
        self.wlstm = nn.Linear(self.input_size*2*64, self.output_size)
        ...
        self.lstm1 = nn.LSTM(32, 32,batch_first=True,bidirectional=True,num_layers=3)
        self.lstm2 = nn.LSTM(2, 256, batch_first=True, bidirectional=False, num_layers=2,dropout=0.3)
    ...
    def forward(self, x):
        ...
        h0 = torch.randn(2, mysize, 256).cuda()
        c0 = torch.randn(2, mysize, 256).cuda()
        # c0 = torch.randn(1, mysize, 32).cuda()
        # output(seq_len, batch, hidden_size * num_directions)
        y2 = self.lstm2(y2,(h0,c0))
        # y2 = y2[0].transpose(1, 2)
        y2 = y2[0].reshape(mysize, -1)
        y2 = self.wlstm(y2)
        # y2 = self.w13(y2)
        # y = x + y2
        # y1 = y.reshape(-1, 32, 2)
        # y1 = y1.transpose(1, 2)
        # y1 = self.lstm1(y1, (h0, c0))
        # # y2 = y2[0].transpose(1, 2)
        # y1 = y1[0].reshape(-1, 128)
        # y = self.wlstm(y1)
        # y = y + y3
        # y = self.w1(y)
        # y = self.w13(y)
