...
class ResNet(nn.Module):
    ...
    def __init__(self, block, blocks_num, num_classes=1000, include_top=True, norm_layer=None):
        ...
        self.rga2 = RGA_Module(in_channel=512, size=20, use_spatial=True, use_channel=True)
        self.layer3 = self._make_layer(block, 256, blocks_num[2], stride=2)
        self.rga3 = RGA_Module(in_channel=1024, size=20, use_spatial=True, use_channel=True)
        self.layer4 = self._make_layer(block, 512, blocks_num[3], stride=2)
        self.rga4 = RGA_Module(in_channel=2048, size=20, use_spatial=True, use_channel=True)
        if self.include_top:
...
def resnet50_fpn_backbone(pretrain_path="",
    ...
    layers_to_train = ['rga4', 'rga3', 'rga2', 'rga1', 'conv1'][:trainable_layers]
    ...
    return_layers = {f'rga{k}': str(v) for v, k in enumerate(returned_layers)}
