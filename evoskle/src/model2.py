...
class CVAE_Linear(nn.Module):
    def __init__(self, size, latent_size = 512, num_samples=1, alpha=1, cvae_num_stack=1):
        ...
        self.inp_post = nn.Linear(32*3 + 32*3, size)
        ...
        self.fc_dec = nn.Linear(32 * 3, self.size - self.latent_size)
        ...
        self.fc_out = nn.Linear(size, 32*3)
    ...
    def forward(self, x, y):
        ...
        out = torch.zeros(self.num_samples,self.batch_size,32*3).cuda()
        # out = torch.zeros(self.num_samples, self.batch_size, 32 * 3)
        out_gsnn = torch.zeros(self.num_samples, self.batch_size, 32 * 3).cuda()
        # out_gsnn = torch.zeros(self.num_samples, self.batch_size, 32 * 3)
