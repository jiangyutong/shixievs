...
from libs.evolution.genetic2 import evolution
...
def visualize(initial_population, evolved_population):
    ...
    from libs.evolution.genetic import show3Dpose
    def get_zmin(pose):
...
def main():
    ...
    if opt.generate:
        ...
        for k2d in data_dic.keys():
            (sub, act, fname) = k2d
            if sub == 5:
                one.append(data_dic[k2d])

        one = np.concatenate(list(one), axis=0)
