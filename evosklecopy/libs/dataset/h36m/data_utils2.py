...
def get_dict_2d(train_dict_3d, test_dict_3d, rcams, ncams, opt):
    ...
    dataloader = DataLoader(dataset=deal_dataset, batch_size=256, shuffle=True)
    train_dict_2d_targ = project_to_cameras2(train_dict_3d, rcams, ncams=ncams)
    test_dict_2d_train = {}
    test_dict_2d_targ = project_to_cameras2(test_dict_3d, rcams, ncams=ncams)
    train_dict_2d_train = {}
    ...
    #                 print("i:{} key[{}] Epoch:{} Loss：{}".format(i,key, batch_idx, loss))
    #             j = j + 1
    # for m in range(3):
    #     if m==1:
    #         print("m:::::::::::::::{}".format(m))
    ...
    # mystep1 = list(range(1, 50, 5))
    # mystep2 = list(range(50, 100, 20))
    # mystep = mystep1 + mystep2
    # scheduler = torch.optim.lr_scheduler.MultiStepLR(optim, milestones=[5000, 7000], gamma=0.2)
    # j = 0
    # # for m in range(3):
    # #     if m==1:
    # #         print("m:::::::::::::::{}".format(m))
    ...
    test_dict_2d_pred = {}
    now = datetime.datetime.now()
    stpath = str(now) + "_" + "jyt_fine_2d_xianx2d4"
    save2Dpath = os.path.join(opt.data_dir, stpath)
    if not os.path.exists(save2Dpath):
        os.mkdir(save2Dpath)
    save2Dpath = save2Dpath + "/" + "model"
    torch.save(mymodel, save2Dpath)
    # mymodel=torch.load('/media/zlz422/82BEDFE2BEDFCD33/jyt/project/EvSkeleton/data/human3.6M/2021-10-24 18:44:39.801745_jyt_fine_2d/model')
    mymodel.eval()
    for key in train_dict_2d.keys():
        # mymodel=mymodel.cuda()
        data_train = train_dict_2d[key]
    ...
    for key in test_dict_2d.keys():
        ...
        # print("Lossall:{}".format(lossall/threeDcameras.shape[0]))
