...
def get_dict_2d(train_dict_3d, test_dict_3d, rcams, ncams, opt):
    ...
    optim = torch.optim.Adam(mymodel.parameters(), lr=0.0003, weight_decay=0.0)
    ...
    for key in train_dict_2d_targ.keys():
        ...
        for batch_idx in range(1, epoch + 1):
            ...
            loss=0.3*loss2+0.3*loss1+0.7*loss3
            ...
            if batch_idx == epoch:
                ...
                keynew.append(key[2] + "-sh")
                keynew = tuple(keynew)
                train_dict_2d_pred[keynew] = prediction
                print("train key{} Loss3: {}".format(key, loss3))
            # logging
            if batch_idx % 50 == 0:
    ...
    j = 0
    test_dict_2d_pred = {}
    for key in test_dict_2d_train.keys():
        lossall = 0
        # scheduler.step()
        # if j != 0:
        #     adjust_lr(optim, 0.002)
        print("lr:{}".format(optim.state_dict()['param_groups'][0]['lr']))
        for batch_idx in range(1, epoch + 1):
            data_train = test_dict_2d_train[key]
            data_targ = test_dict_2d_targ[key]
            threeDcameras = test_dict_3d_convert[key]
            # get2ddata(mymodel, optim,data_train,data_targ,threeDcameras,opt,j,epoch,key)
            ...
            if batch_idx == epoch:
                ...
                keynew = tuple(keynew)
                # prediction = [ predictionitem.detach().numpy() for predictionitem in prediction]
                test_dict_2d_pred[keynew] = prediction
                print("test key{}  Loss3: {}".format( key, loss3))
            # logging
            if batch_idx % 50 == 0:
                now = datetime.datetime.now()
                print("[{}] key:{} Train Epoch: {} Loss:  {} Loss3: {}".format(now, key, batch_idx, loss, loss3))
    ...
    #             print("FTrainLoss2: {}".format(loss2))
    #             prediction = prediction.detach().cpu().numpy()
    #             # prediction = [ predictionitem.detach().numpy() for predictionitem in prediction]
