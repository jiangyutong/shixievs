...
import libs.model.model9  as model
...
def get_dict_2d(train_dict_3d, test_dict_3d, rcams, ncams, opt):
    ...
    train_dict_2d_pred = {}
    test_dict_2d_pred = {}
    # mymodel = torch.load('/media/zlz422/82BEDFE2BEDFCD33/jyt/project/EvSkeleton/data/human3.6M/2021-11-15 16:55:06.508504_jyt_fine_2d_every/model')
    mymodel.train()
    mymodel = torch.load(
        '/media/zlz422/82BEDFE2BEDFCD33/jyt/project/EvSkeleton/data/human3.6M/2021-11-16 17:05:11.603034_jyt_fine_2d_every/model')
    ...
    for batch_idx in range(1, epoch + 1):
        ...
        while data_targ is not None:
            ...
            # loss = loss2 + 0.1*loss1+0.6*loss3
            loss = 0.6 * loss11 + 0.1 * loss12 + 0.6 * loss4

            loss.backward()
            optim.step()
            lossall = lossall + loss
            loss4all = loss4all + loss4
            lossfall = lossfall + loss11
            losscall += loss12
            data_targ, data_train, threeDcameras, f, c = prefetcher.next()
