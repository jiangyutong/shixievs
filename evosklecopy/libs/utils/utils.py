...
def load_ckpt(opt,save_dir):
    ...
    stats = np.load(os.path.join(save_dir, 'stats.npy'),allow_pickle=True).item()
    if opt.cuda:
        cascade.cuda()
    ...
    for batch_idx, batch in enumerate(eval_loader):
        ...
        for sample_idx in range(batch_size):
            for stage_idx in range(num_stages):
                ...
                show3Dpose2(prediction_stages[stage_idx][sample_idx], ax1, add_labels=False, plot_dot=True)
                # viz.show3Dpose(skeleton_3d_gt[sample_idx], ax, gt=True)
                axes.append(ax)      
        adjust_figure(left = 0.05, right = 0.95, bottom = 0.05, top = 0.95,
                      wspace = 0.3, hspace = 0.3)              
