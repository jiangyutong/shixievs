...
class FCModel(nn.Module):
    def __init__(self,
        ...
        self.batch_norm2 = nn.BatchNorm1d(self.input_size)
        self.res_blocks = []
        # for l in range(2):
        #     self.res_blocks.append(ResidualBlock(self.input_size,
        #                                          self.p_dropout,
        #                                          leaky=self.leaky))
        ...
        self.w3 = nn.Linear(self.output_size, self.output_size)
        if self.leaky:
            self.relu = nn.LeakyReLU(inplace=True)
        else:
            self.relu = nn.ReLU(inplace=True)
