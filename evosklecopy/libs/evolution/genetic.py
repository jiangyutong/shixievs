...
H36M_NAMES[18] = 'LElbow'#肘部
H36M_NAMES[19] = 'LWrist'#Wrist
...
def xyz2spherical(xyz):
    ...
    # return in r, phi, and theta (elevation angle from z axis down) r、 φ和θ
    return_value = np.zeros(xyz.shape, dtype=xyz.dtype)
    xy = xyz[:,:,0]**2 + xyz[:,:,1]**2
    return_value[:,:,0] = np.sqrt(xy + xyz[:,:,2]**2) # r
    return_value[:,:,1] = np.arctan2(np.sqrt(xy), xyz[:,:,2]) # phiφ
    return_value[:,:,2] = np.arctan2(xyz[:,:,1], xyz[:,:,0]) #thetaθ
    return return_value 
